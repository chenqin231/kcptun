...
description: 查看所有已学习的 instinct 及其置信度
---
...
1. Detect current project name (via `git rev-parse --show-toplevel` basename or `$PWD` basename)
2. Read instinct files from:
   - `~/.claude/homunculus/instincts/global/` (通用经验)
   - `~/.claude/homunculus/instincts/projects/<project>/` (项目专属经验)
   - `~/.claude/homunculus/instincts/inherited/` (继承的经验)
3. Display them grouped by scope (global/project/inherited) and domain with confidence bars
